...
public class VpnProfile implements Serializable {
    ...
    public int[] mServerPorts = new int[]{1194};
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        int numPorts = mServerPorts.length;
        for ( int index = 0; index < numPorts; index++){
        	// Server Address
        	cfg += "remote ";
        	cfg += mServerName;
        	cfg += " ";
        	cfg += mServerPorts[index];
        	if (mUseUdp){
        		cfg += " udp\n";
        	} else {
        		cfg += " tcp-client\n";
        ...
        if ( nonNull(mTLSCipher) ){
        	cfg += "tls-cipher " + mTLSCipher + "\n";
        }
